...
MANIFEST = "cron_tracking/daily_combined/pending/9a7d935f-95e6-40ac-8dfc-a6dd1d9a3eb7/run_manifest.json"
OUT_DIR = "cron_tracking/daily_combined/pending/9a7d935f-95e6-40ac-8dfc-a6dd1d9a3eb7"
...
def render(pkg):
    ...
    if pkg.get("spoiler_warning"):
        lines.append("SPOILER WARNING: Yes")
    lines.append("")
    ...
    for i, s in enumerate(pkg["sources"], 1):
        # F-fix (this rebuild, 2026-08-26): some source entries added during the
        # Law #165 correction round never carried a 'date' key. Rather than
        # crash or fabricate a date, render an explicit, honest placeholder.
        date = s.get("date", "date not recorded in manifest")
        lines.append(f"{i}. {s['claim']} — {s['url']} ({date})")
    lines.append("")
...
for pkg in m["packages"]:
    ...
    fname = f"{OUT_DIR}/email_{pkg['slot']}.txt"
