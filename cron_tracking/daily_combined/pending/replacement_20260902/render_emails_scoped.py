...
m = json.load(open("cron_tracking/daily_combined/pending/replacement_20260902/run_manifest.json"))

def render(pkg):
    slot = pkg["slot"].upper()
    post_date = pkg["post_date"]
    subject = f"REPLACEMENT | {slot} | {pkg['show']} | {post_date} | {pkg['youtube_title']}"
    ...
    if pkg["format_type"] == "EPISODE_MOMENT":
...
for pkg in m["packages"]:
    ...
    fname = f"cron_tracking/daily_combined/pending/replacement_20260902/email_{pkg['slot']}.txt"
